# Paisley Grammar and Language Support

This extension provides language support for the [Paisley](https://github.com/ZacharyWesterman/paisley) scripting language.
The current supported version is at least v1.21.11, and should work correctly even for more recent versions (if there's no new syntax).
If the system has Paisley installed, then that version will be used. Otherwise, a fallback build (bundled with this extension) is used.

Features include:
- Syntax highlighting.
- Syntax validation.
- Type checking.
- Hover info for code entities.
- Warnings for poorly formed code or deprecated language features.
- Identifying dead code blocks.
- Semantic coloring for certain code entities (constant vars, etc).
